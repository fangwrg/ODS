...
# reservoirs and their optimized storm location and angle
csv_in = base_dir + os.sep + r'ScriptResults\3_Run_HMS_Elliptical_Design_Storm_Optimization' \
                           r'\3b_Trinity_XYThetaOpt_100yr48hr_elev.csv '
...
peak_elev = []  # create list 'peak_elev' to store peak elevations after each loop
...
reservoir_list = []  # create list to store reservoir names
...
with open(csv_in, 'r') as f:
    ...
    for column in csv.reader(f):
        ...
        reservoir_list.append(column[0])

# Loop through all the reservoirs with their previously optimized storm locations
counter = 0
for i in range(len(reservoir_list)):
    # Assign the current reservoir name
    reservoir = reservoir_list[i]
    # Assign the optimized values for the current reservoir
    x_opt = x_opt_list[i]
    y_opt = y_opt_list[i]
    theta_opt = theta_opt_list[i]

    print("\n***** For " + reservoir + ", storm to be moved to \n   " + str(x_opt) + ", " + str(y_opt) + ", at " + str(
        theta_opt) + " degrees. *****\n")
    ...
    storm_final = output_dir + os.sep + '4b_Storm_' + reservoir + '.tif'
    ...
    hyetograph_dir = os.path.dirname(dss_in) + os.sep + reservoir + '_' + output_name
    ...
    folium.Choropleth(
        ...
        fill_opacity=0.8,
        line_opacity=0.1,
        legend_name="Avg. Precipitation Depths: " + reservoir,
        smooth_factor=0,
        Highlight=True,
        line_color="#0000",
        show=True,
    ...
    # Save map to html
    mapa.save(output_dir + os.sep + '4c_Storm_Map_' + reservoir + '_' + output_name + '.html')

    counter = counter + 1

    print("\n***** Reservoir " + str(counter) + " of " + str(len(reservoir_list)) + " complete! *****\n")

# Write an out csv file with the storm parameters and peak elevation for each reservoir
header = ['Reservoir', 'PeakElev', 'X', 'Y', 'Theta', 'CenterDepth']
rows = zip(reservoir_list, peak_elev, x_opt_list, y_opt_list, theta_opt_list, depths_max_hr)
...
print("****************************************************************")
print("Finalized Design Storm runs have been made for " + str(len(reservoir_list)) + " reservoirs.")
