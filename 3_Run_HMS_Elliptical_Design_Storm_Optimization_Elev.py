...
# Variable 'reservoir_list' is a tuple of reservoir names (strings) that need to be exactly those in the HMS models
# The user needs to prepare the reservoir names of interest; reservoir_list must contain at least 2 reservoirs
reservoir_list = ('Joe_Pool_Lake', 'Ray_Roberts_Lake')

# Variable 'runs' is the limit of optimization runs for each reservoir. Along with HMS simulation time,
# it determines the total run time. Testing on a 15,000 sqmi basin indicates 300 is a good number, which
# gave a total run time of ~ 1 hour per reservoir
runs = 300
...
output_name = frequency + duration + "_elev"
...
# 'csv_out_2' is the name of this script's output file with optimized storm locations and peak elevs for each reservoir
...
class SpotpySetup(object):
    ...
    # and extract peak elevation from the output DSS.
    def simulation(self, x):
        # Introduce some global variables
        global base_dir, ellipses_shapefile, x0, y0, theta0, hyetograph_time_step, output_dir, \
            software_dir, dss_in, reservoir, subbasin_col_name, hms_model_dir, \
        ...
        storm_final = temp_dir + os.sep + 'Storm_' + reservoir + '.tif'
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ EXTRACT PEAK ELEVATION ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

        # Extract the reservoir peak elevation from the DSS file holding model output
        print("\n***** Reading the output DSS file... *****\n")
        pathname = "//" + reservoir.upper() + "/ELEVATION//" + ctrl_spec_time_interval.upper() + "/RUN:" + hms_run + "/"
        ...
        # Save reservoir peak elevation value as csv
        row = [round(max_val, 1), round(x[0], 2), round(x[1], 2), round(x[2])]
                ...
                for line in run_log:
                    if line.startswith("ERROR"):
                        ...
                        with open(script_error_log, write_mode) as error_log:
                            ...
                            error_log.write("The below simulation results for " + reservoir + " are invalid: \n")
        ...
        # so that the peak elevation is maximized
...
# Create a file that will store optimized parameters and elevations for each reservoir
with open(csv_out_2, 'w') as csv_file_2:
    q_writer = csv.writer(csv_file_2, lineterminator='\n')
    q_writer.writerow(['Reservoir', 'PeakElev', 'X', 'Y', 'Theta'])

# Go through all the reservoirs
counter = 0
for i in range(len(reservoir_list)):
    # Assign the current reservoir name
    reservoir = reservoir_list[i]
    # Prepare a csv file to log peak elev, lon, lat, theta for ALL runs (to be used to create an optimization map)
    csv_out = temp_dir + os.sep + "3a_" + reservoir + "_RunLog_" + output_name + ".csv"
    with open(csv_out, 'w') as csvFile:
        writer = csv.writer(csvFile, lineterminator='\n')
        writer.writerow(['PeakElev', 'X', 'Y', 'Theta'])

    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ OPTIMIZE FOR THE CURRENT RESERVOIR ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    sampler = spotpy.algorithms.sceua(SpotpySetup(), dbname=temp_dir + os.sep + '3_Spotpy_DB_SCEUA_' + reservoir,
                                      dbformat='csv', db_precision=np.float32)
    sampler.sample(runs, ngs=4)
    collected = gc.collect()

    if os.path.exists(temp_dir + os.sep + '3_Spotpy_DB_SCEUA_' + reservoir + '.csv'):
        os.remove(temp_dir + os.sep + '3_Spotpy_DB_SCEUA_' + reservoir + '.csv')

    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ WRITE AN OUTPUT CSV FILE w/ OPTIMIZED INFO~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    df_runlog = pd.read_csv(csv_out, dtype={'PeakElev': np.float32, 'X': np.float32,
                                            'Y': np.float32, 'Theta': np.float32})
    max_elev_index = df_runlog['PeakElev'].idxmax()
    max_elev_row = df_runlog.loc[max_elev_index, :]
    with open(csv_out_2, 'a+') as csv_file_2:
        q_writer = csv.writer(csv_file_2, lineterminator='\n')
        q_writer.writerow([reservoir_list[i], round(max_elev_row[0], 3), round(max_elev_row[1], 2),
                           round(max_elev_row[2], 2), round(max_elev_row[3])])
    ...
    gdf_runlog.to_file(output_dir + os.sep + '3c_Optimization_Points_' + reservoir + '_' + output_name + '.shp')
    ...
    p0 = gdf_runlog['PeakElev'].min().min()
    p75 = gdf_runlog['PeakElev'].quantile(0.75)
    p95 = gdf_runlog['PeakElev'].quantile(0.95)
    p100 = gdf_runlog['PeakElev'].max().max()

    colors = []

    try:
        for index, row in gdf_runlog.iterrows():
            current = row['PeakElev']
            if current <= p75:
                colors.append('green')
            elif current <= p95:
    ...
    feature_group2 = folium.FeatureGroup(name='Simulated Storm Centers: ' + reservoir)
    ...
    for index, row in gdf_runlog.iterrows():
        ...
        simulation_elev = f"{(round(row['PeakElev'], 2)):,}"
        html = f"""
        ...
        <p> Lat: {simulation_lat}<br>
            ...
            This simulated storm center led to a peak elevation of <strong>{simulation_elev}ft</strong> at {reservoir}.
        <p>
        """.format(simulation_lat=simulation_lat, simulation_lon=simulation_lon, simulation_theta=simulation_theta,
                   simulation_elev=simulation_elev, reservoir=reservoir)
    ...
    mapa.keep_in_front(hover, feature_group2)

    # Add heatmap to map
    data_heatmap = gdf_runlog[['Lat', 'Lon', 'PeakElev']].values.tolist()
    ...
    <div class='legend-scale'>
      <ul class='legend-labels'>
        <li><span style='background:purple;opacity:0.7;'></span>Optimized Peak Elevation At Reservoir</li>
        <li><span style='background:red;opacity:0.7;'></span>Very High Peak Elevation At Reservoir</li>
        <li><span style='background:yellow;opacity:0.7;'></span>High Peak Elevation At Reservoir</li>
        <li><span style='background:green;opacity:0.7;'></span>Low Peak Elevation At Reservoir</li>
    ...
    # Save map to html
    mapa.save(output_dir + os.sep + '3d_Optimization_Map_' + reservoir + '_' + output_name + '.html')

    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ CREATE COLORRAMPS IN AN EXCEL WORKBOOK ~~~~~~~~~~~~~~~~~~~~~~~~~~

    # Combine log .csv files into one Excel workbook; add color gradient; bold font the highest peak elevation
    ...
    if counter == 0:
        ...
        pd.read_csv(csv_out).to_excel(writer, sheet_name=reservoir, index=False)
        writer.save()
        writer.close()
    ...
    print("\n***** Reservoir " + str(counter) + " of " + str(len(reservoir_list)) + " complete! *****\n")
...
print("****************************************************************")
print("Storm locations have been optimized for " + str(len(reservoir_list)) + " reservoirs.")
